...
var can, cell_length,alive_pieces = [],g,selected_grid = [8,8,9,9], turn = "white",selected_piece,possible_route=[],waitForPromo = false;
...
function Pawnpromotion(key,turn,x,y) {
  ...
  if (waitForPromo){
    ...
    if (key == "1"){
      yy = new Rook(x,y,turn) // all new
    }
  
    else if (key == "2"){
      yy = new Horse(x,y,turn) // all new
    }
  
    else if (key == "3"){
      yy = new Bishop(x,y,turn) // all new
    }
  
    else if (key == "4"){
      yy = new Queen(x,y,turn)
    }
  
    alive_pieces.push(yy)
    a.grid[y][x] = yy
  
    index = alive_pieces.indexOf(a.grid[y][x]) // remove old
    alive_pieces.splice(index)
...
function keyPressed() {
  ...
  else if (PROMO.includes(key) && pawnUpgrade){
    Pawnpromotion(key,turn,x,y)
  }
}
...
function move_pieces(new_location,piece,alive_pieces){
  ...
  if (a.grid[new_location[1]][new_location[0]] == 0){
    ...
    // move piece to new location in the grid
      ...
      if (new_location[1] == 7 || new_location[1] == 0){
        // prompt() vs ingame shit
      }
    }

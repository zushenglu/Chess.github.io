...
var can, cell_length,alive_pieces = [],g,selected_grid = [8,8,9,9], turn = "white",selected_piece,possible_route=[], waitForPromo = false, promo_loc = [];
...
const VER = 8, HOR = 8, PROMO = ['1','2','3','4'];
var x_adjust
var y_adjust
...
function Pawnpromotion(key,turn,x,y) {
var color1
  if (turn == "black"){
    color1 = "white"
  }
    ...
    if (key == "1"){
      yy = new Rook(x,y,color1) // all new
    }
  
    else if (key == "2"){
      yy = new Horse(x,y,color1) // all new
    }
  
    else if (key == "3"){
      yy = new Bishop(x,y,color1) // all new
    }
  
    else if (key == "4"){
      yy = new Queen(x,y,color1)
    }
  
    var dead;
    dead = a.grid[y][x]; // pawn is dead
    dead.alive = false;

    index = alive_pieces.indexOf(dead) // remove old
    alive_pieces.splice(index,1)

    a.grid[dead.y][dead.x] = yy
    alive_pieces.push(yy)
...
function keyPressed() {
  ...
  else if (PROMO.includes(key) && waitForPromo){
    Pawnpromotion(key,turn,promo_loc[0],promo_loc[1])
  }
}
...
function move_pieces(new_location,piece,alive_pieces){
  ...
  if (piece.name == "pawn" && (new_location[1] == 7 || new_location[1] == 0)){
    // ingame shit
    promo_loc = [new_location[0],new_location[1]]
    waitForPromo = true;
  }
}
...
function other_threat_loc_king(x,y,turn){ // change shits here so i wont kill myself
  ...
  for (let i = x-1;i>-1;i--){
    ...
    if (y-i+x < 8 && !bf){
      if (grid[y-i+x][i] instanceof Piece){
        if (grid[y-i+x][i].color != turn && (grid[y-i+x][i].name == "bishop" || grid[y-i+x][i].name == "king" && Math.abs(i-x) == 1 || grid[y-i+x][i].name == "queen" || grid[y-i+x][i].name == "pawn" && grid[y-i+x][i].color == "white" && x-i == 1)){
          a4.push([i,y-i+x])
          threat_pieces.push(grid[y-i+x][i])
          threat_grid.push(a4);
          bf = true;
